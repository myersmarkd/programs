...
public class ModifyProductMenuController implements Initializable {
    ...
    private void productSaveBtnAction(ActionEvent event) throws IOException {
        ...
        for(int i = 0; i < Product.getListTracker().size(); ++i) {
            if(Product.getListTracker().get(i) == id) {
                Product.deleteListTracker(i);
                Product.deleteAssocPart(Product.getAllAssocParts().get(i));
            }
        }
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        Product modifyProduct = MainMenuController.getSelectedProduct();
        productIdTextBox.setText(Integer.toString(modifyProduct.getId()));
        productNameTextBox.setText(modifyProduct.getName());
        productInvTextBox.setText(Integer.toString(modifyProduct.getStock()));
        productPriceTextBox.setText(Double.toString(modifyProduct.getPrice()));
        productMaxTextBox.setText(Integer.toString(modifyProduct.getMax()));
        productMinTextBox.setText(Integer.toString(modifyProduct.getMin()));
        ...
        for(int i : Product.getListTracker()) {
            if(i == modifyProduct.getId()) {
                associatedPartsList.add(Product.getAllAssocParts().get(counter));
                ++counter;
            }
        }
        associatedPartsTable.setItems(associatedPartsList);
